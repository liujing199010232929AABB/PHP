    ...
    public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler, $parameter)
    {
        ...
        if ($_attr[ 'type' ] !== 'tag') {
            if ($compiler->php_handling === Smarty::PHP_REMOVE) {
                ...
                $output =
                    preg_replace_callback('#(<\?(?:ajax2|=)?)|(<%)|(<script\s+language\s*=\s*["\']?\s*ajax2\s*["\']?\s*>)|(\?>)|(%>)|(<\/script>)#i',
            ...
            $compiler->has_code = true;
            if (!($compiler->smarty instanceof SmartyBC)) {
                $compiler->trigger_template_error('{ajax2}{/ajax2} tags not allowed. Use SmartyBC to enable them', null,
                                                  true);
            }
